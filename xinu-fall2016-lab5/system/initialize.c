...
struct createinfo infotab[NPROC];
bool8 sys_proc[NPROC];
...
void freelist_snapshot(){

	struct memblk *cur = memlist.mnext;
	int count = 1; //including copy of memlist struct
	while(cur!=NULL){
		cur = cur->mnext;
		count++;
	}
	snapshot_size = count;
	head = (struct memblk *)getmem(2*sizeof(uint32)*count);
	head->mnext = memlist.mnext;
	head->mlength = memlist.mlength;

	struct memblk *cur_1 = head+1;
	cur = memlist.mnext;
	
	while(cur != NULL){
		cur_1->mnext = cur->mnext;
		cur_1->mlength = cur->mlength;
		cur = cur->mnext;
		//kprintf("%p\n", (void *)cur_1);
		cur_1++;
	}
