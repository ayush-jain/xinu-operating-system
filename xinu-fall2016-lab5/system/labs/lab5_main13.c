...
void myproc1(void)
{
	...
	for(i = 0; i < VECSIZE; i++) {
		myproc1buf[i] = mainbuf1[i] + mainbuf2[i];
		sleepms(10);
	}
...
void myproc2(void)
{
	...
	for(i = 0; i < VECSIZE; i++) {
		myproc2buf[i] = mainbuf1[i] * mainbuf2[i];
		sleepms(10);
	}
...
process	main(void)
{
	...
	if(ta_tc_restart_flag == 0) {
		...
		resume(create(myproc1, 4096, 6, "MYPROC1", 0));
		resume(create(myproc2, 4096, 5, "MYPROC2", 0));

		sleepms(100);
		ta_tc_restart_flag = 1;
		kprintf("MAIN RESTARTING\n");
		restart(0);
	} else {

		for(i = 0; i < VECSIZE; i++) {
			mainbuf1[i] = 2 * i;
			mainbuf2[i] = 2 * (VECSIZE - i);
		}
