...
/* in file elf_parser.c */
extern void* parse(int fd);
extern void read_elf_header(int32 fd, Elf32_Ehdr *elf_header);
extern bool8 is_ELF(Elf32_Ehdr eh);
extern void print_elf_header(Elf32_Ehdr elf_header);
extern void read_section_header_table(int32 fd, Elf32_Ehdr eh, Elf32_Shdr sh_table[]);
extern char* read_section(int32 fd, Elf32_Shdr sh);
extern void print_section_headers(int32 fd, Elf32_Ehdr eh, Elf32_Shdr sh_table[]);
extern void print_symbol_table(int32 fd, Elf32_Ehdr eh, Elf32_Shdr sh_table[], uint32 symbol_table);
extern void print_symbols(int32 fd, Elf32_Ehdr eh, Elf32_Shdr sh_table[]);
extern void* elf_load(int fd, char *start, Elf32_Ehdr *hdr, Elf32_Shdr *shdr, Elf32_Sym *symtab, uint32 symtbl_idx);
extern char* load_file(int fd, Elf32_Ehdr eh, Elf32_Shdr sh_table[]);
extern int elf_do_reloc(int fd, char* start, Elf32_Ehdr *hdr, Elf32_Shdr sh_table[], Elf32_Rel *rel, Elf32_Shdr *reltab, Elf32_Sym *symtab, uint32 symtbl_idx);
extern int elf_get_symval(int fd, char* start, Elf32_Ehdr *hdr, Elf32_Shdr sh_table[], Elf32_Sym *symtab, int strtbl_idx, uint32 symtbl_idx, uint32 idx);
extern int store_xinuinfo(int gfd);
extern char *elf_lookup_symbol(const char *name);
